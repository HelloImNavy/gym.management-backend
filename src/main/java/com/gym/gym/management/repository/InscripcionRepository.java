...
public interface InscripcionRepository extends JpaRepository<Inscripcion, Long> {
	...
	@Query("SELECT new com.gym.gym.management.dto.InscripcionDTO(i.id, i.miembro.id, i.miembro.nombre, i.miembro.apellidos, a.id, a.nombre, i.fechaAlta, i.fechaBaja) " +
		       "FROM Inscripcion i JOIN i.actividad a " +
		       "WHERE i.miembro.id = :idMiembro")
		List<InscripcionDTO> findInscripcionesConMiembro(@Param("idMiembro") Long idMiembro);

    Inscripcion findByMiembroAndActividad(Miembro miembro, Actividad actividad);
}
